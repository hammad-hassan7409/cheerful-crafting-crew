...
import { LogIn, Play, Image as ImageIcon, Loader2, ShieldAlert, MessageSquare, ZoomIn, ZoomOut, RotateCcw } from "lucide-react";
...
function ProtectedMedia({ children, type = "video", scale = 1, onScaleChange, isZoomEnabled = false }) {
    ...
    // Pinch to zoom logic
    useEffect(() => {
        if (!internalRef || type !== "image" || !isZoomEnabled)
            return;
        let initialDist = 0;
    ...
    return (<div ref={setInternalRef} className="relative h-full w-full select-none overflow-hidden touch-none" onContextMenu={handleContextMenu} onDragStart={(e) => e.preventDefault()}>
      <div className="w-full h-full transition-transform duration-200 ease-out flex items-center justify-center" style={{
...
const VideoPreview = memo(function VideoPreview({ mediaUrl }) {
    ...
    return (<ProtectedMedia type="video">
      {showLoader && (<div className="absolute inset-0 flex items-center justify-center bg-muted/50 backdrop-blur-sm z-10 transition-opacity duration-300">
      ...
      {signedUrl && (<video src={signedUrl} className={`max-w-full max-h-full w-auto h-auto object-contain grayscale-[0.5] group-hover:grayscale-0 transition-all duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`} muted playsInline loop preload="metadata" controlsList="nodownload" onCanPlay={() => {
                setIsLoading(false);
            }} onLoadedData={() => {
                setIsLoading(false);
            }} onMouseEnter={(e) => !isLoading && e.currentTarget.play()} onMouseLeave={(e) => {
                if (!isLoading) {
                    e.currentTarget.pause();
                    e.currentTarget.currentTime = 0;
                }
            }}/>)}
      {!isLoading && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
...
}, function VideoDialog({ mediaUrl, productName }) {
    const [isLoading, setIsLoading] = useState(true);
    const { url: signedUrl } = useSignedUrl(mediaUrl);
    const showLoader = !signedUrl || isLoading;
      ...
      <DialogContent className="max-w-4xl bg-black border-white/10 p-0 overflow-hidden">
        ...
        <div className="relative w-full bg-black flex items-center justify-center min-h-[300px]">
          <ProtectedMedia type="video">
            {showLoader && (<div className="absolute inset-0 flex items-center justify-center z-10">
                <div className="flex flex-col items-center gap-4 w-1/2">
                  <Loader2 className="h-10 w-10 animate-spin text-primary"/>
                  <span className="text-xs font-bold uppercase tracking-tighter text-white/60">Buffering Preview</span>
                </div>
              </div>)}
            {signedUrl && (<video src={signedUrl} className={`max-w-full max-h-[80vh] w-auto h-auto bg-black transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`} controls autoPlay playsInline preload="auto" controlsList="nodownload" onCanPlay={() => {
                setIsLoading(false);
            }} onLoadedData={() => {
                setIsLoading(false);
            }}/>)}
...
}, function ImageZoomDialog({ signedUrl, productName }) {
    const [zoom, setZoom] = useState(1);
    return (<div className="relative w-full h-full flex items-center justify-center">
      <ProtectedMedia type="image" scale={zoom} onScaleChange={setZoom} isZoomEnabled={true}>
        {signedUrl && (<img src={signedUrl} alt={productName} className="max-w-full max-h-[90vh] w-auto h-auto object-contain"/>)}
      </ProtectedMedia>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 p-2 rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 z-20">
        <Button variant="ghost" size="icon" onClick={() => setZoom(prev => Math.max(prev - 0.25, 1))} disabled={zoom <= 1} className="h-10 w-10 rounded-xl text-white hover:bg-white/10 disabled:opacity-30">
          <ZoomOut className="h-5 w-5"/>
        </Button>
        <div className="w-12 text-center text-[10px] font-bold text-white uppercase tracking-widest">
          {Math.round(zoom * 100)}%
        </div>
        <Button variant="ghost" size="icon" onClick={() => setZoom(prev => Math.min(prev + 0.25, 3))} disabled={zoom >= 3} className="h-10 w-10 rounded-xl text-white hover:bg-white/10 disabled:opacity-30">
          <ZoomIn className="h-5 w-5"/>
        </Button>
        <div className="w-[1px] h-6 bg-white/10 mx-1"/>
        <Button variant="ghost" size="icon" onClick={() => setZoom(1)} disabled={zoom === 1} className="h-10 w-10 rounded-xl text-white hover:bg-white/10 disabled:opacity-30">
    ...
    const handleSendToEditor = useCallback((productName) => {
      ...
      <header className="fixed top-0 z-50 w-full border-b border-border/40 bg-background/60 backdrop-blur-xl">
        <div className="container mx-auto flex h-20 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={() => navigate({ to: "/login" })}>
      ...
      <main className="container mx-auto px-6 pt-32 pb-20">
        ...
        {categories?.map((category) => {
            ...
            if (!categoryProducts || categoryProducts.length === 0)
                return null;
            return (<section key={category.id} className="mb-24">
              ...
              <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                {categoryProducts.map((product) => (<Link key={product.id} to="/products/$productId" params={{ productId: product.id }} className="block outline-none">
                    <ProductCard product={product} whatsappNumber={settings?.["whatsapp_number"]}/>
                  </Link>))}
        ...
        {(!products || products.length === 0) && (<div className="flex h-[40vh] flex-col items-center justify-center text-center space-y-4">
    ...
    return (<div className="group relative flex flex-col rounded-2xl border border-border/50 bg-card/50 transition-all hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 overflow-hidden">
      <div className="relative w-full overflow-hidden bg-muted flex items-center justify-center min-h-[200px]">
        {product.media_type === "video" ? (<VideoPreview mediaUrl={product.media_url}/>) : (<Dialog>
            <DialogTrigger asChild>
              <button className="h-full w-full cursor-zoom-in relative group/image">
                <ProtectedMedia type="image">
                  {signedUrl && (<img src={signedUrl} alt={product.name} className="max-w-full max-h-full w-auto h-auto object-contain transition-transform duration-700 group-hover:scale-110"/>)}
                </ProtectedMedia>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="mt-auto flex items-end justify-between gap-4">
          ...
          {product.media_type === "video" ? (<VideoDialog mediaUrl={product.media_url} productName={product.name}/>) : null}
        </div>

        <Button className="w-full mt-6 h-12 bg-primary hover:bg-primary/80 text-white font-bold rounded-xl shadow-lg shadow-primary/20 transition-all hover:shadow-primary/40 flex items-center justify-center gap-2" onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
