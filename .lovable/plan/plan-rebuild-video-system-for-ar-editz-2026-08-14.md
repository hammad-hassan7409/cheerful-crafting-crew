...
> **1. Do not use** `preload="metadata"` **as a mandatory fixed setting. Configure video loading specifically for fast playback without forcing unnecessary full-file downloads.**
>   **2. Do not use excessively short-lived signed URLs. The signed URL must remain valid for the expected viewing session and must not expire while the video is playing. Refresh it only when genuinely necessary, without unnecessarily replacing the video source.**